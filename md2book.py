...
/* Disable scrim when cover_scrim: false */
.cover.no-scrim::before { display: none; }
...
def build_cover(meta, overrides, image_uri=None):
    ...
    subtitle = overrides.get('subtitle') or meta.get('subtitle') or ''
    blurb    = overrides.get('blurb')    or meta.get('blurb')    or ''
    ...
    if blurb:    lines.append(f'    <p class="blurb">{blurb}</p>')
    if author:   lines.append(f'    <span class="author">{author}</span>')
    lines.append('  </div>')
    return '\n'.join(lines)
